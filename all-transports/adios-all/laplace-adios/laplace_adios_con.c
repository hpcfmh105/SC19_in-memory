...
int main (int argc, char ** argv) 
{
   ...
   uint64_t nn = nx * ny;
    ...
    if(has_keep ==1){
        if(transport_minor == DSPACES){
            adios_init ("adios_xmls/laplace_dataspaces.xml", comm);
        }

        else if(transport_minor == DIMES){
            adios_init ("adios_xmls/laplace_dimes.xml", comm);
        }

        else if(transport_minor == FLEXPATH){
            adios_init ("adios_xmls/laplace_flexpath.xml", comm);
        }
        else{
...
*/
    ...
    ADIOS_VARINFO * v = adios_inq_var (f, "var_2d_array");
	...
	uint64_t size_tot = slice_size * v->dims[1] ;
...
#endif
        run_analysis(data, size_tot/2, lp, sum_vx,sum_vy);
#ifdef V_T
      VT_end(analysis_id);
#endif
        t4 = MPI_Wtime();
