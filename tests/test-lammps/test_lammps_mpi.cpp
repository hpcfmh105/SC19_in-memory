...
int main(int argc, char *argv[])
{
	...
	int mpisize = 0;
	MPI_Type_size(MPI_INT, &mpisize);
	printf("mpisize: %d\n", mpisize);

    MPI_Type_size(MPI_LONG_LONG, &mpisize);
    printf("LONG LONG mpisize: %d\n", mpisize);

    MPI_Type_size(MPI_LMP_TAGINT, &mpisize);
    printf("LMP TAGINT LONG mpisize: %d\n", mpisize);


    printf("before LAMMPS\n");
    LAMMPS* lps = new LAMMPS(0, NULL, comm);
    printf("before load input file\n");
...

  {
     ...
     //extract "value"
        ...
        if(rank == 0){
        printf("step %d i have %d lines %d atoms, sim time %.3f extract time %.3f\n",
                timestep,
                nlocal,
				natoms,
                t2-t1,
