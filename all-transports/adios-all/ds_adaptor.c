...
void ds_delay(int milliseconds)
{
    long pause;
    clock_t now,then;
...
#define debug_1
status_t get_common_buffer(uint8_t transport_minor,int timestep,int ndim, uint64_t bounds[6], int rank, char * var_name, void **p_buffer,size_t elem_size, double *p_time_used, MPI_Comm* pcomm){
    ...
    */
    ...
    PDBG( "try to acquired the read lock %s for step %d", lock_name, timestep);
      ...
      VT_begin(ds_lock_id);
#endif
    dspaces_lock_on_read(lock_name, pcomm);
#ifdef V_T
      VT_end(ds_lock_id);
#endif
	...
	ret_get= -11;
    while( ret_get == -11){  
    	PDBG( "before dspaces/dimes get var %s, rank %d, for step %d \n", var_name, rank, timestep);
		if(transport_minor == DIMES){
			#ifdef NODIMES
				
        		PDBG( "dataspeces no dimes can not call dimes get  \n");
       		 	TRACE();
        		return(S_FAIL);
			#else
		...
		if(ret_get == -11 || ret_get == -EAGAIN){
			delay(500);
		}
	
    	PDBG( "after dspaces/dimes get var %s rank %d, for step %d, return %d \n", var_name, rank, timestep, ret_get);
...
#endif
    dspaces_unlock_on_read(lock_name, pcomm);
#ifdef V_T
    VT_end(ds_unlock_id);
#endif
...
status_t put_common_buffer(uint8_t transport_minor, int timestep,int ndim, uint64_t bounds[6], int rank,char * var_name, void  **p_buffer,size_t elem_size, double *p_time_used, MPI_Comm* pcomm){
    ...
    */
    ...
    PDBG( "try to acquired the write lock %s for step %d", lock_name, timestep);
      ...
      VT_begin(ds_lock_id);
#endif
    dspaces_lock_on_write(lock_name, pcomm);
#ifdef V_T
      VT_end(ds_lock_id);
#endif
    ...
    if(transport_minor == DIMES){
         ...
         /*   if(S_OK != dimes_put_sync_group(lock_name, 0)){
            ...
            PDBG( "before dimes put at step %d", timestep);
        ret_put = dimes_put(var_name, timestep, elem_size, ndim, lb, ub,  *p_buffer);
		 PDBG( "after dimes put rank %d, ret_put %d at step %d",rank, ret_put, timestep);		
	#endif
        //    PDBG( "before dimes sync at step %d", timestep);
//		dimes_put_sync_all();
          //  PDBG( "after dimes sync rank %d at step %d",rank, timestep);
    }
    else{
            PDBG( "before dataspaces put var %s at step %d", var_name, timestep);
        ret_put = dspaces_put(var_name, timestep, elem_size, ndim, lb, ub,  *p_buffer);

            PDBG( "before dspaces  sync at step %d", timestep);
		dspaces_put_sync();
            PDBG( "after dspaces  sync var %s ret_put=%d rank %d at step %d", var_name, ret_put, rank, timestep);
    }
	}
...
//	MPI_Barrier(*pcomm);
    ...
    // now we can release region lock
      ...
      VT_begin(ds_unlock_id);
#endif
    dspaces_unlock_on_write(lock_name, pcomm);
#ifdef V_T
      VT_end(ds_unlock_id);
#endif
