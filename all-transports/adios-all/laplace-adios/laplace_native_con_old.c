...
int main (int argc, char ** argv) 
{
 ...
 //   if(S_OK != get_common_buffer(transport_minor, 0,3, bounds_size,rank, var_size, (void **)& global_size, sizeof(long), &time_comm, &comm)){
   //       TRACE();
    //      MPI_Abort(comm, -1);
      //  }
 ...
 slice_size = slice_size * nodeedge;
    ...
    while(timestep < nsteps){
        ...
        if(S_OK != get_common_buffer(transport_minor, timestep,3, bounds,rank, var_name, (void **)&data, elem_size, &time_comm, &comm)){
          TRACE();
          MPI_Abort(comm, -1);
        }
        // analysis
