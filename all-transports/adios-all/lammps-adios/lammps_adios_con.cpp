...
int main (int argc, char ** argv){
    ...
    if(adios_read_init_method (method, comm, "verbose=2") !=0){
...
//    ADIOS_FILE * afile = adios_read_open (filename, method, comm, ADIOS_LOCKMODE_CURRENT, -1);
	if(rank == 0)
		printf("before adios read open: rank 0 filename %s\n ", filename);
ADIOS_FILE * afile = adios_read_open (filename, method, comm, ADIOS_LOCKMODE_CURRENT, -1);
    //ADIOS_FILE * f = adios_read_open (filename, method, comm, ADIOS_LOCKMODE_NONE, 0);
    if(rank == 0)
        printf("before adios read open: rank 0 is streaming:  %d ", afile->is_streaming);
    ...
    int size_one = 5; 
	ADIOS_VARINFO *array = adios_inq_var( afile, "var_2d_array");
    int nlines;
    uint64_t slice_size = array->dims[1]/nprocs;
	if( array != 0){
		nlines = (int) slice_size * array->dims[2];
	}else {
		PERR("query array in adios fail");
	}
    ...
    while(adios_errno != err_end_of_stream){       
        ...
        if(!t){
            long arr_size = sizeof(double) * nelem;
...
#endif
		if (rank == 0)
			printf("calc_msd %ld, size one %d, step %d \n ", nlines, size_one, ii);
        calc_msd(msd, t, nlines, size_one, ii);
#ifdef V_T
      VT_end(analysis_id);
#endif
        t2 =MPI_Wtime(); 
  ...
  /*
	...
	// Dan modify not finalize dataspaces due to stuck of dataspaces
