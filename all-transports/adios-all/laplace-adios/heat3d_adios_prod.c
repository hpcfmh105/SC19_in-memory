  ...
  if(transport_major == ADIOS_DISK || transport_major == ADIOS_STAGING){
      ...
      if(transport_major == ADIOS_DISK){
        ...
        if(S_OK != adios_adaptor_update_avail_version(comm, step_index_file, step, maxStep)){
			...
			put_common_buffer(transport_minor, step,2, bounds_size,me, var_size, (void **) & bounds, sizeof(long), &time_comm);
			put_common_buffer(transport_minor, step,2, bounds,me , var_name, (void **)&x, elem_size, &time_comm);
		}
  else{
